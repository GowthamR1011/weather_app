...
const ToggleSwitch: React.FunctionComponent<IToggleSwitchProps> = (props) => {
  ...
  return(
    <div className="m-3 inline-flex items-center gap-2">
    { props.IconOff && <props.IconOff />}
    {props.labelOff && <label htmlFor={props.id} className="text-white text-sm cursor-pointer">{props.labelOff}</label> }
    ...
    {props.IconOn && <props.IconOn />}
    {props.labelOn && <label htmlFor={props.id} className="text-white text-sm cursor-pointer">{props.labelOn}</label> }
  </div>
  );
};
